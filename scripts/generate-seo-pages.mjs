...
function articleJsonLd(cluster, url) {
...
<html lang="en">
  <head>
    ...
    <script type="application/ld+json">${articleJsonLd(cluster, url)}</script>
    <style>
...
for (const cluster of news.clusters || []) {
  ...
  sitemapItems.push({
    ...
    lastmod: news.updatedAt ? new Date(news.updatedAt).toISOString() : cluster.publishedAt
  });
}
...
console.log(`Generated ${sitemapItems.length} SEO brief pages.`);
