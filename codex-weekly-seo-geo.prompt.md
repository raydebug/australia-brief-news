...
Goal: once per week, review and improve 4News SEO and GEO (generative engine optimisation) across all supported languages, based on the site's product position, current Australian news/search trends, multilingual reader intent, and current machine-readable site assets.
...
- Make the multilingual positioning reflect the three core reader jobs: simple English learning, finding conversation topics, and quickly understanding Australia.
...
Weekly checklist:
1. Read `README.md`, `index.html`, `public/llms.txt`, `public/sitemap.xml`, `public/robots.txt`, `public/about.html`, `public/editorial-policy.html`, `public/corrections.html`, and the latest `public/news.*.json` language files.
2. Search the web for current Australian news search themes and multilingual reader intent around learning English, conversation topics, and understanding Australia. Use this only to identify language gaps and query intent, not to copy competitors.
3. Check whether homepage metadata still matches actual product features and current reader intent in English, Simplified Chinese, Traditional Chinese, Sinhala, Japanese, Korean, Vietnamese, Thai, and any newly added languages.
4. Check structured data in `index.html` for validity, conservative wording, and accurate multilingual language declarations.
...
7. Check `public/llms.txt` describes the site accurately, links every available machine-readable language data file, reflects the three reader jobs, and avoids overclaiming.
...
10. If new language files are added, update `hreflang`, `llms.txt`, structured data `inLanguage`, relevant metadata, and any app language selectors.
